...
export default function DocumentDesigner({ business, onSave, onBack }) {
 ...
 return <div>
  ...
  <div className="bb-card"><div className="bb-section-title">Customize everything</div><div className="designer-control-grid">
   ...
   <Field label="Document title"><input className="bb-input" value={style.documentTitle||""} placeholder="Leave blank for Invoice / Estimate" onChange={e=>setStyle({...style,documentTitle:e.target.value})}/></Field>
   <Field label="Watermark"><input className="bb-input" value={style.watermarkText||""} placeholder="e.g. PAID / DRAFT" onChange={e=>setStyle({...style,watermarkText:e.target.value})}/></Field>
  ...
  <div className="designer-checks">{[["showLogo","Show logo"],["showTaxSummary","Show tax summary"],["showAmountWords","Show amount in words"],["showBank","Show bank details"],["showSignature","Show signature box"],["showTerms","Show terms & conditions"],["showHsn","Show HSN/SAC column"],["showQty","Show quantity column"],["showRate","Show price column"],["showGst","Show GST column"]].map(([key,label])=><label key={key}><input type="checkbox" checked={style[key]!==false} onChange={e=>setStyle({...style,[key]:e.target.checked})}/>{label}</label>)}</div>
